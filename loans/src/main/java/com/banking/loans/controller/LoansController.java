...
public class LoansController {
    ...
    public ResponseEntity<ResponseDto> createLoan(@RequestParam @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile Number must be 10 digits") String mobileNumber) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ResponseDto.builder().statusCode(LoansConstants.STATUS_201).statusMsg(LoansConstants.MESSAGE_201).build());
    }
    ...
    public ResponseEntity<ResponseDto> updateLoanDetails(@RequestBody @Valid LoansDto loansDto) {
        ...
        return isUpdated ? ResponseEntity.status(HttpStatus.OK).body(ResponseDto.builder().statusCode(LoansConstants.STATUS_200).statusMsg(LoansConstants.MESSAGE_200).build()) : ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body(ResponseDto.builder().statusCode(LoansConstants.STATUS_417).statusMsg(LoansConstants.MESSAGE_417_UPDATE).build());
    }
    ...
    public ResponseEntity<ResponseDto> deleteLoanDetails(@RequestParam @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile Number must be 10 digits") String mobileNumber) {
        ...
        return isDeleted ? ResponseEntity.status(HttpStatus.OK).body(ResponseDto.builder().statusCode(LoansConstants.STATUS_200).statusMsg(LoansConstants.MESSAGE_200).build()) : ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body(ResponseDto.builder().statusCode(LoansConstants.STATUS_417).statusMsg(LoansConstants.MESSAGE_417_DELETE).build());
    }
}
